...
import { generateCommitMessage, pickIndependentGroup } from './openai';
...
export async function handleUnstagedChanges(config: GitConfig): Promise<void> {
  while (true) {
    ...
    const spinnerPick = ora('Selecting an independent commit topic...').start();
    let selected: TopicGroup | null = null;
    try {
      selected = await pickIndependentGroup(config, items);
      spinnerPick.succeed(`Selected topic: ${selected.topic} (${selected.items.length} items)`);
    } catch (err: any) {
      spinnerPick.fail('Selection failed: ' + String(err));
      // Fallback: commit first item only
      if (items.length > 0) {
        selected = { topic: items[0].file, items: [items[0].id] };
      } else {
        selected = null;
      }
    }
    ...
    const groupItems = selected.items.map(id => itemMap.get(id)).filter(Boolean) as ChangeItem[];

    console.log(chalk.yellow('\n---'));
    console.log(chalk.bold(`Topic: ${selected.topic}`));
    console.log(chalk.dim('(Preview of the topic changes)'));
    ...
    if (!confirm) {
      console.log(chalk.gray('Skipping this suggestion for now.'));
      continue;
    }
